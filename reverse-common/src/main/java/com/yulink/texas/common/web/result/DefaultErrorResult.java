...
import com.yulink.texas.common.web.enums.BusinessExceptionEnum;
...
/**
 * @desc 默认全局错误返回结果 备注：该返回信息是spring boot的默认异常时返回结果{@link DefaultErrorAttributes}，目前也是我们服务的默认返回结果
 */
@Builder
@AllArgsConstructor
@NoArgsConstructor
...
public class DefaultErrorResult implements Result {
    ...
    private static final long serialVersionUID = -465020020238884539L;
    /**
    ...
    /**
     * 我们系统内部自定义的返回值编码，{@link ErrorCode} 它是对错误更加详细的编码
     *
    ...
    public static DefaultErrorResult failure(ErrorCode resultCode, Throwable e, HttpStatus httpStatus, Object errors) {
        DefaultErrorResult result = DefaultErrorResult.failure(resultCode, e, httpStatus);
        result.setErrors(errors);
        return result;
    }

    public static DefaultErrorResult failure(ErrorCode resultCode, Throwable e, HttpStatus httpStatus) {
...
//        result.setException(e.getClass().getName());
    ...
    public static DefaultErrorResult failure(BusinessException e) {
        ...
        DefaultErrorResult defaultErrorResult = DefaultErrorResult
            .failure(e.getResultCode() == null ? ErrorCode.SUCCESS : e.getResultCode(), e, HttpStatus.OK, e.getData());
        if (StringUtil.isNotEmpty(e.getMessage())) {
            defaultErrorResult.setMessage(e.getMessage());
            defaultErrorResult.setMsg(defaultErrorResult.getMessage());
        }
