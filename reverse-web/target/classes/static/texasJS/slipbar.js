...
var DrawSlipBar = {
    ...
    OnMouseMove: function (evt) {
        ...
        var cx = evt.clientX - texasCanvas.offsetLeft;
        var cy = evt.clientY - texasCanvas.offsetTop;
        // 如果事件位置在矩形区域中,调试会导致的浏览器坐标错误
        if (cx >= DrawSlipBar.position.x && cx <= DrawSlipBar.position.x + DrawSlipBar.position.w
            && cy >= DrawSlipBar.position.y && cy <= DrawSlipBar.position.y + DrawSlipBar.position.h
        ) {
            setTimeout(() => {
